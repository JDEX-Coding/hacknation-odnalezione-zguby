...
class CLIPHandler:
    ...
    def __init__(self, model_name: str = "openai/clip-vit-base-patch32", text_model_name: str = "sentence-transformers/clip-ViT-B-32-multilingual-v1"):
        """
        Initialize CLIP handler.

        Args:
            model_name: Name of the CLIP model to use from HuggingFace (for images)
    ...
    def load_model(self):
        ...
        try:
            logger.info(f"Loading CLIP model for images: {self.model_name}")
            self.processor = CLIPProcessor.from_pretrained(self.model_name)
            self.model = CLIPModel.from_pretrained(self.model_name)
            self.model.to(self.device)
            self.model.eval()

            logger.info(f"Loading Multilingual CLIP model for text: {self.text_model_name}")
