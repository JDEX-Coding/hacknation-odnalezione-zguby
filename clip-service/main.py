...
Also exposes an HTTP API for real-time embedding generation.
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error("Max retries reached. RabbitMQ unavailable.")
                    # Don't exit, just log error, so API can still run (maybe)
                    # Although for consumption we need RabbitMQ
...
async def embed_text(request: EmbedRequest):
    ...
    try:
        embedding = service_instance.clip_handler.encode_text(request.text)
